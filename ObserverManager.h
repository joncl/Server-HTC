...
#ifndef _OBSERVERMANAGER_h
#define _OBSERVERMANAGER_h
...
public:
    ...
    static void subscribe(O* observer) {
        static_assert(std::is_base_of<ObserverBase, O>::value,
                      "All observers must inherit from ObserverBase");
        if (observer == nullptr) {
            return;
        }
    ...
    static void notify(void (P::*method)(MethodArgumentTypes...), ActualArgumentTypes... args) {
        std::vector<std::pair<P*, ObserverBase::id_t>> observers;
...
private:
    static std::unordered_map<ObserverBase::id_t, void (*)(ObserverBase*)> unsubscribeMap;
};
